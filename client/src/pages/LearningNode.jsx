...
const resources = [
  { type:'video',  title:'React state deep dive — hooks in practice',        duration:'42 min',      typeLabel:'Video' },
  { type:'article',title:'A visual guide to useEffect dependencies',          duration:'12 min read',  typeLabel:'Article' },
  { type:'docs',   title:'Official React docs — Managing state',              duration:'Reference',    typeLabel:'Documentation' },
]

const concepts = [
  'useState and the render cycle',
  'useEffect and cleanup functions',
  'Derived state vs. stored state',
  'Lifting state up the tree',
  ...
  return (
    <div style={{ background:'var(--color-bg-base)',fontFamily:'var(--font-sans)',minHeight:'100vh' }}>
      ...
      <nav style={{ height:52,borderBottom:'1px solid var(--color-border-subtle)',display:'flex',alignItems:'center',padding:'0 24px',gap:12,background:'rgba(10,10,15,0.9)',backdropFilter:'blur(12px)',position:'sticky',top:0,zIndex:100 }}>
        ...
        <span style={{ color:'var(--fg-muted)',fontSize:13 }}>React Fundamentals</span>
        <span style={{ color:'var(--fg-muted)',fontSize:13 }}>/</span>
        <span style={{ color:'var(--fg)',fontSize:13,fontWeight:500 }}>Week 2</span>
      </nav>
      ...
      <main style={{ maxWidth:760,margin:'0 auto',padding:'40px 24px 80px' }}>
        ...
        <h1 style={{ fontFamily:'var(--font-display)',fontSize:32,fontWeight:700,letterSpacing:'-0.02em',lineHeight:1.15,color:'var(--fg)',marginBottom:8 }}>
          State management &amp; the component lifecycle
        </h1>
        <p style={{ fontSize:15,color:'var(--fg-muted)',lineHeight:1.6,marginBottom:28 }}>
          Understand how React tracks and updates state, when components re-render, and how to use hooks to manage side effects cleanly.
        </p>

        {/* Meta */}
        <div style={{ display:'flex',alignItems:'center',gap:20,marginBottom:20,flexWrap:'wrap' }}>
          {[
            { icon:<ClockIcon/>, text:'Est. 3h 20m' },
            { icon:<DocsIcon size={14}/>, text:'3 resources' },
            { icon:<CheckIcon/>, text:'5 concepts' },
          ].map((m,i) => (
        ...
        <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:40 }}>
          {concepts.map((c,i) => (
            <div key={i} style={{ display:'flex',alignItems:'flex-start',gap:10,padding:'14px 16px',background:'var(--color-bg-surface-2)',border:'1px solid var(--color-border-default)',borderRadius:'var(--radius-lg)',transition:'border-color 0.15s ease,transform 0.15s ease',cursor:'default' }}
              ...
              <div style={{ width:6,height:6,borderRadius:'50%',background:i===4?'var(--color-gold-500)':'var(--color-purple-500)',flexShrink:0,marginTop:5 }}/>
            ...
            return (
              <div key={i} style={{ display:'flex',alignItems:'center',gap:16,padding:'18px 20px',background:'var(--color-bg-surface-2)',border:'1px solid var(--color-border-default)',borderRadius:'var(--radius-lg)',transition:'border-color 0.15s ease,transform 0.15s ease,box-shadow 0.15s ease',cursor:'default' }}
                ...
                <div style={{ width:40,height:40,borderRadius:'var(--radius-md)',background:ic.bg,color:ic.color,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0 }}>
                  {r.type==='video' ? <VideoIcon/> : r.type==='article' ? <ArticleIcon/> : <DocsIcon/>}
                </div>
                <div style={{ flex:1,minWidth:0 }}>
                  ...
                  <div style={{ display:'flex',alignItems:'center',gap:10 }}>
                    <span style={{ fontSize:11,fontWeight:600,letterSpacing:'0.06em',textTransform:'uppercase',padding:'2px 7px',borderRadius:'var(--radius-sm)',background:ic.bg,color:ic.color }}>{r.typeLabel}</span>
                    <span style={{ fontSize:12,color:'var(--fg-muted)',display:'flex',alignItems:'center',gap:4 }}><ClockIcon size={11}/>{r.duration}</span>
                  </div>
                </div>
                <button onClick={() => openResource(i)} style={{ display:'flex',alignItems:'center',gap:6,padding:'7px 14px',borderRadius:'var(--radius-md)',border:'1px solid var(--color-border-default)',background:'transparent',color:resourceOpened[i]?'var(--color-purple-400)':'var(--fg-secondary)',fontSize:13,fontWeight:500,cursor:'pointer',transition:'var(--transition-fast)',whiteSpace:'nowrap',flexShrink:0,fontFamily:'var(--font-sans)',borderColor:resourceOpened[i]?'var(--color-purple-500)':'var(--color-border-default)' }}
        ...
        <div style={{ display:'flex',flexDirection:'column',gap:12,paddingTop:8 }}>
          <button onClick={() => navigate('/quiz')} style={{ display:'flex',alignItems:'center',justifyContent:'center',gap:8,width:'100%',padding:'16px 24px',background:'var(--color-purple-500)',color:'#fff',fontSize:15,fontWeight:600,fontFamily:'var(--font-sans)',border:'none',borderRadius:'var(--radius-lg)',cursor:'pointer',transition:'background 0.15s ease,transform 0.15s ease,box-shadow 0.15s ease',letterSpacing:'-0.01em' }}
